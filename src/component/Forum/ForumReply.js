...
class ForumReply extends Component {
    ...
    render() {
        return ( < div >
            ...
            <
            Col > < Card style = {
                { width: '25rem', marginTop: '20px', border: '2px solid grey' } } >
            <
            Card.Body >
            <
            Card.Title > < img src = { pic2 }
            style = {
                { width: '30px' } }
            className = "rounded mr-2"
            alt = "" / > Anushka Praveen < /Card.Title> <
            ...
            /Card> {
                this.state.replies.slice(0, this.state.visiblereply).map((reply) => ( <
                    Card style = {
                        { width: '400px', marginBottom: '30px', marginTop: '20px', border: '1px solid grey' } } >
                    <
                    Card.Header >
                    <
                    img src = { pic2 }
                    style = {
                        { width: '30px' } }
                    className = "rounded mr-2"
                    alt = "" / > Anushka Praveen < small style = {
                        { float: 'right' } } > < /small></Card.Header >
                    ...
                    size = 'sm'
                    onClick = {
                        () => this.handleDelete(reply.id) } > Delete < /Button> <
                    /Card.Body> <
                    /Card>
                ))
            } <
            div class = "col-md-12 p-3 text-center" > {
                this.state.visiblereply < this.state.replies.length && < button type = "button"
                class = "btn btn-outline-info"
                onClick = { this.loadmore } > Read more < /button>}</div >
                <
                /Col>
                ...
                Card.Text >
                <
                textarea placeholder = "Please Type reply here"
                style = {
                    { width: '460px' } }
                ...
                onClick = { this.handleReply } > Post < /Button> <
                /Card>
